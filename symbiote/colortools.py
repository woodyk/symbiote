...
class ColorConverter:
    ...
    def get_color_format(self, input_color):
        if isinstance(input_color, tuple):
            output_format = 'rgb'
        elif isinstance(input_color, str) and input_color.startswith('#'):
            output_format = 'hex'
        elif isinstance(input_color, str) and input_color.lower() in webcolors.CSS3_NAMES_TO_HEX:
            output_format = 'color'
        else:
            raise ValueError("Invalid input color")
    ...
    def get_contrasting_colors(self, input_color, output_format=None, num_colors=2):
        if output_format is None:
            output_format = self.get_color_format(input_color)
        ...
        if brightness < 128:
            contrast_color = (255, 255, 255)  # Pure white
            hex_contrast = self.convert(contrast_color, 'hex')
            colors = [hex_contrast] * num_colors
        else:
            contrast_color = (0, 0, 0)  # Pure black
            hex_contrast = self.convert(contrast_color, 'hex')
            colors = [hex_contrast] * num_colors
    ...
    def get_monochromatic_colors(self, input_color, output_format=None,  num_colors=5):
        ...
        for i in range(num_colors):
            ...
            r_new, g_new, b_new = colorsys.hls_to_rgb(h, s, l_new)
            rgb_new = (int(r_new*255), int(g_new*255), int(b_new*255))
            formatted_color = self.convert(rgb_new, output_format)
            colors.append(formatted_color)
        return colors
    ...
    def display_color_chart(self, input_color, num_colors=5):
        ...
        for label, method in color_methods:
            # Some methods might not accept num_colors, so use try-except
            try:
                method_colors = method(input_color, output_format='hex', num_colors=num_colors)
            except TypeError:
                method_colors = method(input_color, output_format='hex')
            ...
            for hex_color in method_colors:
                if hex_color:
                    ...
                    rgb_color = self.convert(hex_color, 'rgb')
                    print(f"{rgb_color} {hex_color} {color_name}")
                    print(f"{colored_block} RGB: {rgb_color} Hex: {hex_color} Name: {color_name}")

            # Adjust plot background for Contrasting colors
            if label == 'Contrasting':
                # Determine if the contrast color is white or black
                if all(color.lower() == '#ffffff' for color in method_colors):
                    facecolor = 'k'  # Black background
                else:
                    facecolor = 'w'  # White background
            else:
                facecolor = 'w'  # Default white background

            # Create a matplotlib chart
            fig, ax = plt.subplots(1, 1, figsize=(10, 2), dpi=80, facecolor=facecolor, edgecolor='k')
