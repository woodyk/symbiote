...
class utilities():
    ...
    def learnFilesXX(self, path, batch_size=128, save_dir='./gpt2_finetuned', save_freq=10):
        # Download the tokenizer files
        self.downloadTokenizerFiles()
        ...
        # Train the model on each batch of data
        for i, file in enumerate(file_list):
            # Use extractText() to read the file
            ...
            dataset = load_dataset('text', data_files='/tmp/train_data.txt')['train']

            # Tokenize the dataset
            tokenized_dataset = dataset.map(lambda examples: tokenizer(examples['text']), batched=True, remove_columns=['text'])
            ...
            try:
                ...
                print(f"Error training model on file {file}: {e}")
                continue
