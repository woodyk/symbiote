...
class Terminal(Widget, can_focus=True):
    def __init__(self, send_queue, recv_queue, ncol, nrow):
        ...
        self.stream = pyte.ByteStream(self._screen)
...
class TerminalEmulator(App):
    """Terminal emulator app using pyte and Textual."""
    ...
    def open_terminal(self):
        """Create the PTY and start bash."""
        pid, fd = pty.fork()
        if pid == 0:
            argv = shlex.split("bash")
            env = dict(TERM="xterm-256color", LC_ALL="en_GB.UTF-8", COLUMNS=str(self.ncol), LINES=str(self.nrow))
